...
object Ceki_Euler11_V1 {
  ...
  //type Dim2IntList = List[Array[Int]]
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48"""
  ...
  def toArray(grid: String): Dim2IntArray = {
    val lines = grid.split("[\\r\\n]+")

    val dim2StrArray = lines.map {
      line =>
        line.split(" ")
    }
    return dim2StrArray.map {
      line =>
        line.map(_.toInt)
    }
  }

  def asVerticalLines(gridAsArray: Dim2IntArray): Dim2IntArray = {
    val height = gridAsArray.length;
    val width = height
    val verticalLines = for (j <- 0 until width) yield {
      val intArray = new Array[Int](height)
      for (i <- 0 until height) {
        intArray(i) = gridAsArray(i)(j)
      }
      intArray
    }
  ...
  def maxOfFourByFourMultiplicaiton(line: Array[Int]): Int = {
    val last = line.length - 4;
    val multiplicationOfFourByFourGrouping = for {i <- 0 to last}
    yield {
      line.slice(i, i + 4).reduceLeft(_ * _)
    };
    multiplicationOfFourByFourGrouping.max
  }

  def findMax(view: Dim2IntArray) = {
    view.map(maxOfFourByFourMultiplicaiton _).max
  }
